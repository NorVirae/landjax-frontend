import type { NextPage } from 'next'
import Head from 'next/head'
import Image from 'next/image' 
import Navbar from '../src/components/Navbar'
import HouseImg from '../src/assets/24.png'


const HouseCard = () => {
  return (
    <section className='future__house-card'>
              <button className='future__btn-sale'>Sale</button>
              
              <Image alt='house image' src={HouseImg} className='future__card-img'/>
              <div className='future__card-body'>
                <h3 className='future__card-title'>
                  Emene new Extension
                </h3>
                <span className='future__card-price'>
                  2000 BTC
                </span>

                <p className='future__card-desc'>
                  the house is closer to you at the side where things they make sense
                </p>

                <div className='future__profile-container'>
                  <Image alt='future' src={HouseImg} height="50rem" width={"50rem"} className="future__profile-img" />
                  Jane Cooper
                </div>
              </div>
            </section>
  )
}

const Home = () => {
  return (
    <div>
      <Head>
        <title>Land Jax</title>
        <meta name="description" content="Generated by create next app" />
        <link rel="icon" href="/favicon.ico" />
      </Head>


      <main>
        <section className='carousel__container'>
          <Navbar />
          
          <div className='carousel__welcome-screen'>
            <h2 className='carousel__welcome-text'>WELCOME TO LANDJAX YOUR</h2>
            <h3 className='carousel__house-lands'>HOUSES AND LANDS</h3>
            <h4 className='carousel__dream-place'>DREAM PLACE</h4>

          </div>

        </section>
        
        <section className='house__preview'>

          <div className='house__preview-overlay'>

          </div>

          <div className='house__preview-inner'>
            <Image alt="src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>
            <Image src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>
            <Image src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>
            <Image src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>
            <Image src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>
            <Image src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>
            <Image src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>
            <Image src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>
            <Image src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>
            <Image src={HouseImg} height={"250rem"} objectFit={"cover"} width="200rem" className='house__image-preview'/>

          </div>

          <section className='house__intro'>
            <p className='house__live'> WHERE DO YOU LIVE?</p>
            <p className='house__asset'> GET AN ASSET WITH <span className='house__land-annouce'>LANDJAX</span></p>

            <div className='house__form-group'>
              <label className='house__form-label'>Enter an Address, city, Neighborhood, zip code</label>
              <input className="house__form-control" placeholder='Enugu, Nigeria' />
            </div>
            <div className='house__btn-ecom-container'>
              <button className='house__btn-buy-prop'>Buy Properties</button>
              <button className='house__btn-sell-prop'>Sell Properties</button>
            </div>
            

          </section>

        </section>

        <section className='future__container'>
          <div className='future__header'>
            <p className='future__prop-text'>Move into your future Hom, today!</p>
            <button className='future__btn-prop'>
              See all properties
            </button>
          </div>

          <div className='future__houses-card-container'>
            <HouseCard />
            <HouseCard />
            <HouseCard />
            <HouseCard />
            <HouseCard />
            <HouseCard />
          </div>
        </section>

        <section className='explore__market-container'>
          <div className='explore__market-card'>
            <h3 className='explore__header'>Explore by Property type</h3>
            <p className='explore__desc'>
              Bro this is the crazy thing about the whole stuff about creation
            </p>
            <button className='future__btn-prop'>
              See all properties
            </button>
          </div>


          <section className='explore__group-card'>
            <div className='explore__image-group'>
              <Image alt="explore img 1" src={HouseImg} />
              <Image alt="explore img 2" src={HouseImg} />

            </div>

            <p className='explore__explain'>
              We have the most listing and constant update so you'ill never  miss out
            </p>
          </section>
        </section>
          
      </main>

      <footer>
        
      </footer>
    </div>
  )
}

export default Home
